...
function generateStats(text) {
    const charNumber = text.length; // 1) Lunghezza testo in esame.
    const arrayFromClearedString = cleanStringFromPunctuationAndConvertInArray(text); // Leva tutta la punteggitura, lascia gli spazie e divide la stringa in array di parole (con lo spazio come separatore)
    
    //console.log(arrayFromClearedString + "\n\n");
    ...
    const charOccurrency = generateOccurrencyCount([...text]); // Conto le occorrenze di ogni carattere nel testo, nella sua versione originale, quella con spazi e punteggiatura.
